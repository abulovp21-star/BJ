    ...
    try:
        async for msg in ws:
            if msg.type != aiohttp.WSMsgType.TEXT:
                break
            if len(msg.data) > 4000:   # hard cap — reject oversized/garbage frames
                continue
            try:
            ...
            # ── SHOP ──────────────────────────────────────────────────────────
                ...
                await ws.send_json(await shop_payload(uid))

            elif act == "buy_cosmetic":
                code = str(d.get("code", ""))
                catalog = CARD_SKINS if code in CARD_SKINS else (PROFILE_FRAMES if code in PROFILE_FRAMES else None)
                if not catalog:
                    await ws.send_json({"type": "error", "msg": "unknown_item"}); continue
                price = catalog[code]["price"]
                owned = await db_owned(uid)
                if code in owned or price == 0:
                    await ws.send_json({"type": "error", "msg": "already_owned"}); continue
                stars = await db_stars(uid)
                if stars < price:
                    await ws.send_json({"type": "error", "msg": "not_enough_stars",
                                        "have": stars, "need": price}); continue
                ...
                if code in CARD_SKINS:
                    ...
                    if code != "classic" and code not in owned:
                        await ws.send_json({"type": "error", "msg": "not_owned"}); continue
                ...
                await ws.send_json(await leaderboard_payload(mode))
            ...
            # ── READY-CHECK: skip the wait once 3+ players all agree ───────────
            elif act == "ready_skip":
                if not room or room["state"] != "lobby": continue
            ...
            # ── INSURANCE ────────────────────────────────────────────────────
                ...
                if not room or room["state"] != "insurance": continue
            ...
            # ── QUICK COMMENTS / EMOJI (rate-limited, not persisted) ───────────
            elif act == "comment":
                ...
                text = str(d.get("text", ""))[:24]  # hard cap length server-side
