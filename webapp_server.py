...
DEALER_DELAY     = 3.8    # pause between dealer actions
N_DECKS          = 6
MIN_BET          = 100
MAX_BET          = 1_000_000
...
pool          = None
rooms         = {}   # rid → room dict
duel_rooms    = {}   # duel_id (int) → duel room dict, separate from the multiplayer `rooms`
duel_waiting  = {}   # duel_id → set of uids that have opened the mini-app for that duel
sessions      = {}   # sid → uid
...
_invite_times = {}   # uid → list of recent invite_friend_to_play timestamps (spam guard)
INVITE_RATE_MAX = 3
...
async def db_profile(uid, viewer_uid=None):
    ...
    w = u["w_bj"] or 0
    l = u["l_bj"] or 0
    pct = round(100 * w / (w+l), 1) if (w+l) else 0.0
    return {
        "type":       "profile",
        "uid":        uid,
        "name":       u["name"],
...
async def game_end(room):
    ...
    for p in room["players"]:
        ...
        if is_split: await db_stats(p["uid"], p["result2"])
...
# ── DUELS (inline-mode 1v1, separate from the multiplayer `rooms`) ───────────
DUEL_TURN_TIME = 30

async def _fetch_duel_row(duel_id):
    return await pool.fetchrow("SELECT * FROM duels WHERE id=$1", duel_id)

def duel_state_for(dr, viewer_uid):
    """Per-viewer view: you always see your own hand in full; the opponent's
    second card is redacted server-side — the client never receives it."""
    order = dr["order"]
    opp_uid = order[1] if order[0] == viewer_uid else order[0]
    me, opp = dr["players"][viewer_uid], dr["players"][opp_uid]
    def redacted(hand, is_me):
        if is_me or dr["state"] == "done":
            return hand, htot(hand)
        if len(hand) <= 1:
            return hand, None
        shown = [hand[0]] + [{"r": "?", "s": "?"}] * (len(hand) - 1)
        return shown, None
    my_hand, my_total = redacted(me["hand"], True)
    op_hand, op_total = redacted(opp["hand"], False)
    result = dr.get("result")
    my_balance = None
    if result:
        my_balance = result["balance_a"] if viewer_uid == order[0] else result["balance_b"]
    return {
        "type": "duel_state",
        "duel_id": dr["id"], "bet": dr["bet"], "state": dr["state"],
        "your_turn": dr["state"] == "playing" and order[dr["cur"]] == viewer_uid,
        "me":  {"uid": viewer_uid, "name": me["name"], "photo": me.get("photo",""),
                "skin": me.get("skin","classic"), "frame": me.get("frame","none"),
                "hand": my_hand, "total": my_total, "done": me["done"]},
        "opp": {"uid": opp_uid, "name": opp["name"], "photo": opp.get("photo",""),
                "skin": opp.get("skin","classic"), "frame": opp.get("frame","none"),
                "hand": op_hand, "total": op_total, "done": opp["done"]},
        "result": result,
        "my_balance": my_balance,
    }

async def bcast_duel(dr):
    for uid in dr["order"]:
        await send_uid(uid, duel_state_for(dr, uid))

async def duel_next_turn(dr):
    ctask_d(dr)
    order = dr["order"]
    if dr["players"][order[0]]["done"] and dr["players"][order[1]]["done"]:
        await duel_finish(dr); return
    while dr["players"][order[dr["cur"]]]["done"]:
        dr["cur"] = 1 - dr["cur"]
    await bcast_duel(dr)
    dr["turn_task"] = asyncio.create_task(duel_auto_stand(dr, dr["cur"]))

def ctask_d(dr):
    tt = dr.get("turn_task")
    if tt and not tt.done(): tt.cancel()
    dr["turn_task"] = None

async def duel_auto_stand(dr, idx):
    try:
        await asyncio.sleep(DUEL_TURN_TIME)
        if dr["state"] == "playing" and dr["cur"] == idx:
            dr["players"][dr["order"][idx]]["done"] = True
            await duel_next_turn(dr)
    except asyncio.CancelledError:
        pass

async def duel_finish(dr):
    dr["state"] = "done"
    a, b = dr["order"]
    ta, tb = htot(dr["players"][a]["hand"]), htot(dr["players"][b]["hand"])
    bust_a, bust_b = ta > 21, tb > 21
    pot = dr["bet"] * 2
    if bust_a and bust_b:
        winner, reason = None, "both_bust"
        await db_add(a, dr["bet"]); await db_add(b, dr["bet"])
    elif bust_a:
        winner, reason = b, "opp_bust"; await db_add(b, pot)
    elif bust_b:
        winner, reason = a, "opp_bust"; await db_add(a, pot)
    elif ta == tb:
        winner, reason = None, "push"
        await db_add(a, dr["bet"]); await db_add(b, dr["bet"])
    elif ta > tb:
        winner, reason = a, "higher"; await db_add(a, pot)
    else:
        winner, reason = b, "higher"; await db_add(b, pot)
    dr["result"] = {"winner_uid": winner, "reason": reason,
                     "balance_a": await db_bal(a), "balance_b": await db_bal(b)}
    await db_stats(a, "win" if winner==a else ("lose" if winner==b else "push"))
    await db_stats(b, "win" if winner==b else ("lose" if winner==a else "push"))
    await bcast_duel(dr)
    await pool.execute("UPDATE duels SET status='finished' WHERE id=$1", dr["id"])

async def duel_deal(dr):
    dk = dr["deck"]
    for uid in dr["order"]:
        dr["players"][uid]["hand"] = [dk.pop(), dk.pop()]
        dr["players"][uid]["done"] = False
    dr["state"] = "playing"; dr["cur"] = 0
    await bcast_duel(dr)
    dr["turn_task"] = asyncio.create_task(duel_auto_stand(dr, dr["cur"]))

async def duel_act_hit(dr, uid):
    idx = dr["order"].index(uid)
    p = dr["players"][uid]
    p["hand"].append(dr["deck"].pop())
    if htot(p["hand"]) > 21:
        p["done"] = True
        await duel_next_turn(dr)
    else:
        await bcast_duel(dr)
        ctask_d(dr)
        dr["turn_task"] = asyncio.create_task(duel_auto_stand(dr, idx))

async def duel_act_stand(dr, uid):
    dr["players"][uid]["done"] = True
    await duel_next_turn(dr)
...
async def action_handler(req):
    ...
    try:
        ...
        if act == "get_shop":
            ...
            try:
                duel_id = int(d.get("duel_id", 0))
            except Exception:
                duel_id = 0
            drow = await _fetch_duel_row(duel_id)
            if not drow or drow["status"] not in ("accepted", "finished") or \
               uid not in (drow["creator_uid"], drow["opponent_uid"]):
                await send_uid(uid, {"type": "error", "msg": "duel_not_found"})
            else:
                dr = duel_rooms.get(duel_id)
                if dr:
                    await send_uid(uid, duel_state_for(dr, uid))
                else:
                    duel_waiting.setdefault(duel_id, set()).add(uid)
                    await send_uid(uid, {"type": "duel_waiting", "duel_id": duel_id})
                    other = drow["opponent_uid"] if uid == drow["creator_uid"] else drow["creator_uid"]
                    if other in duel_waiting.get(duel_id, set()):
                        a, b = drow["creator_uid"], drow["opponent_uid"]
                        ua, ub = await db_user(a), await db_user(b)
                        new_dr = {
                            "id": duel_id, "bet": drow["bet"], "order": [a, b], "cur": 0,
                            "state": "playing", "deck": make_deck(), "turn_task": None, "result": None,
                            "players": {
                                a: {"name": (ua or {}).get("name","P1"), "photo": "",
                                    "skin": (ua or {}).get("equipped_skin") or "classic",
                                    "frame": (ua or {}).get("equipped_frame") or "none",
                                    "hand": [], "done": False},
                                b: {"name": (ub or {}).get("name","P2"), "photo": "",
                                    "skin": (ub or {}).get("equipped_skin") or "classic",
                                    "frame": (ub or {}).get("equipped_frame") or "none",
                                    "hand": [], "done": False},
                            },
                        }
                        duel_rooms[duel_id] = new_dr
                        await duel_deal(new_dr)

        elif act in ("duel_hit", "duel_stand"):
            try:
                duel_id = int(d.get("duel_id", 0))
            except Exception:
                duel_id = 0
            dr = duel_rooms.get(duel_id)
            if dr and dr["state"] == "playing" and dr["order"][dr["cur"]] == uid:
                if act == "duel_hit": await duel_act_hit(dr, uid)
                else: await duel_act_stand(dr, uid)
            ...
            now = time.time()
            recent = [ts for ts in _invite_times.get(uid, []) if now - ts < INVITE_RATE_WINDOW]
            if len(recent) >= INVITE_RATE_MAX:
                ...
                if status != "friends":
                    ...
                    try:
                        sent = await send_telegram_invite(target, uid, (me or {}).get("name", "Друг"), room_id)
                        if sent:
                            recent.append(now)
                            _invite_times[uid] = recent
                            await send_uid(uid, {"type": "invite_sent", "target_uid": target})
                        else:
                            await send_uid(uid, {"type": "error", "msg": "invite_muted"})
                    except Exception:
...
async def send_telegram_invite(target_uid, from_uid, from_name, room_id):
    """Returns False (and sends nothing) if target_uid has muted invites from
    from_uid within the last 24h; otherwise sends the invite and returns True."""
    muted = await pool.fetchrow(
        "SELECT 1 FROM invite_mutes WHERE muter_uid=$1 AND inviter_uid=$2 AND until_ts>$3",
        target_uid, from_uid, int(time.time()))
    if muted:
        return False
    url = WEBAPP_PUBLIC_URL or "https://t.me"
    buttons = [[{"text": "🎲 Присоединиться", "web_app": {"url": url}}]] if WEBAPP_PUBLIC_URL else []
    buttons.append([{"text": "🔇 Не беспокоить 1 день", "callback_data": f"mute_invite|{from_uid}"}])
    markup = {"inline_keyboard": buttons}
    text = f"🎮 <b>{from_name}</b> приглашает вас сыграть в Blackjack!"
    payload = {"chat_id": target_uid, "text": text, "parse_mode": "HTML", "reply_markup": markup}
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=8)) as s:
