...
REF_VIP_STARS  = 5      # 💫 bot stars for VIP referrer per invite
REF_NORM_STARS = 3      # 💫 bot stars for regular referrer per invite
...
# ── SHOP ───────────────────────────────────────────────────────────────────
# Section 1: pay REAL Telegram Stars (⭐, XTR) → receive 💫 bot stars (bigger packs = better rate)
STAR_PACKS = {
    "sp1": ("💫 50",   50,   50),
    "sp2": ("💫 110",  100,  110),
    "sp3": ("💫 280",  250,  280),
...
# Section 2: pay 💫 bot stars (internal currency, no real payment) → VIP or chips
SHOP_ITEMS = {
...
TX['en'] = {
    ...
    'shop': '🛍 <b>Shop</b>\n\nTwo sections:\n💫 <b>Buy bot stars</b> with real Telegram Stars ⭐\n🛍 <b>Buy VIP / chips</b> with your bot stars 💫',
    'shop_stars_title': '💫 <b>Buy bot stars</b>\n\nPay with real Telegram Stars ⭐ — bigger packs give a better rate.',
    'shop_items_title': '🛍 <b>VIP & Chips</b>\n\nPay with your 💫 bot stars balance: <b>{bs:,}💫</b>',
    'not_enough_stars': '❌ Not enough 💫 bot stars. You have <b>{have:,}💫</b>, need <b>{need:,}💫</b>.\nBuy more with real Telegram Stars via the shop.',
    ...
    'pay_ok_stars': '✅ <b>+{amt:,}💫</b> bot stars added to your balance!',
    ...
    'btn_bonus': '🎁 Bonus',
    'btn_shop': '🛍 Shop',
    'btn_shop_stars': '💫 Buy bot stars (for ⭐)',
...
TX['ru'] = {
    ...
    'shop': '🛍 <b>Магазин</b>\n\nДва раздела:\n💫 <b>Купить звёзды бота</b> за настоящие тг-звёзды ⭐\n🛍 <b>Купить VIP / фишки</b> за звёзды бота 💫',
    'shop_stars_title': '💫 <b>Купить звёзды бота</b>\n\nОплата настоящими тг-звёздами ⭐ — чем больше пакет, тем выгоднее курс.',
    'shop_items_title': '🛍 <b>VIP и фишки</b>\n\nОплата с баланса звёзд бота 💫: <b>{bs:,}💫</b>',
    'not_enough_stars': '❌ Недостаточно звёзд бота 💫. У тебя <b>{have:,}💫</b>, нужно <b>{need:,}💫</b>.\nПополни баланс в магазине за настоящие тг-звёзды.',
    ...
    'pay_ok_stars': '✅ <b>+{amt:,}💫</b> звёзд бота зачислено!',
    ...
    'btn_bonus': '🎁 Бонус',
    'btn_shop': '🛍 Магазин',
    'btn_shop_stars': '💫 Купить звёзды бота (за ⭐)',
...
async def add_stars(uid: int, d: int):
    """Bot's own internal currency (💫 звёзды бота) — separate from real Telegram Stars (⭐)."""
...
async def cmd_givestars(msg: Message):
    ...
    p = (msg.text or "").split()
    if len(p) < 3:
        await send(msg.chat.id, "📋 /givestars <uid|@user> <звёзды бота>"); return
    tu = await _resolve_uid(p[1])
    if not tu: await send(msg.chat.id, "❌ Пользователь не найден"); return
    try: amt = int(p[2])
    except Exception: await send(msg.chat.id, "❌ Некорректное число"); return
...
async def cmd_takestars(msg: Message):
    ...
    p = (msg.text or "").split()
    if len(p) < 3:
        await send(msg.chat.id, "📋 /takestars <uid|@user> <звёзды бота>"); return
    tu = await _resolve_uid(p[1])
    if not tu: await send(msg.chat.id, "❌ Пользователь не найден"); return
    try: amt = int(p[2])
    except Exception: await send(msg.chat.id, "❌ Некорректное число"); return
...
async def payment_ok(msg: Message):
    ...
    # payload format: "bjstar_{code}_{uid}" — real TG Stars purchase, credits bot stars
